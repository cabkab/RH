...
/-
Checking if A_S is a topological ring and locally compact space.
-/
open MeasureTheory Complex ENNReal ContinuousLinearMap

variable (S : Finset ℕ) (hS : ∀ p ∈ S, Nat.Prime p)

-- Check if A_S is a topological ring
#synth IsTopologicalRing (A_S S hS)

-- Check if A_S is locally compact
#synth LocallyCompactSpace (A_S S hS)

/-
Checking if instances for L2(A_S) are automatically inferred.
-/
open MeasureTheory Complex ENNReal ContinuousLinearMap
...
/-
Proving that Schwartz functions on R are in L2(R).
-/
open MeasureTheory Complex ENNReal ContinuousLinearMap

theorem Schwartz_mem_L2_real (f : SchwartzMap ℝ ℂ) :
  MemLp f 2 volume := by
    exact?

/-
Proving that locally constant compactly supported functions on Q_p are in L2(Q_p).
-/
open MeasureTheory Complex ENNReal ContinuousLinearMap

variable (p : ℕ) [Fact p.Prime]

theorem LocallyConstant_CompactSupport_mem_L2_padic (f : ℚ_[p] → ℂ)
  (hlc : IsLocallyConstant f) (hcs : HasCompactSupport f) :
  MemLp f 2 (@dx_p p ‹_›) := by
    have h_integrable : MeasureTheory.Integrable f (dx_p p) := by
      apply_rules [ Continuous.integrable_of_hasCompactSupport, hlc.continuous ];
      constructor;
      -- The measure of a compact set in a locally compact space is finite.
      have h_compact_finite : ∀ K : Set ℚ_[p], IsCompact K → (MeasureTheory.Measure.addHaarMeasure (Zp_compacts p)) K < ⊤ := by
        intro K hK
        exact (by
        apply_rules [ IsCompact.measure_lt_top ]);
      exact h_compact_finite;
    have h_integrable_sq : MeasureTheory.Integrable (fun x => ‖f x‖ ^ 2) (dx_p p) := by
      have h_bounded : ∃ M : ℝ, ∀ x, ‖f x‖ ≤ M := by
        have h_bounded : ∃ M : ℝ, ∀ x ∈ tsupport f, ‖f x‖ ≤ M := by
          exact IsCompact.exists_bound_of_continuousOn ( hcs.isCompact ) ( hlc.continuous.continuousOn );
        exact ⟨ Max.max h_bounded.choose 0, fun x => if hx : x ∈ tsupport f then le_trans ( h_bounded.choose_spec x hx ) ( le_max_left _ _ ) else by rw [ show f x = 0 from Classical.not_not.1 fun hx' => hx <| subset_closure <| by aesop ] ; norm_num ⟩;
      have h_integrable_sq : MeasureTheory.Integrable (fun x => ‖f x‖ * h_bounded.choose) (dx_p p) := by
        exact h_integrable.norm.mul_const _;
      refine' h_integrable_sq.mono' _ _;
      · have h_measurable : Measurable f := by
          exact hlc.continuous.measurable;
        exact Measurable.aestronglyMeasurable ( h_measurable.norm.pow_const 2 );
      · filter_upwards [ ] with x using by simpa [ sq ] using mul_le_mul_of_nonneg_left ( h_bounded.choose_spec x ) ( norm_nonneg ( f x ) ) ;
    rw [ memLp_two_iff_integrable_sq_norm ] ; aesop;
    exact h_integrable.1

/-
Proving that locally constant compactly supported functions on the finite adeles are in L2.
-/
open MeasureTheory Complex ENNReal ContinuousLinearMap

variable (S : Finset ℕ) (hS : ∀ p ∈ S, Nat.Prime p)

theorem LocallyConstant_CompactSupport_mem_L2_finite_adele (f : FiniteAdeleS S hS → ℂ)
  (hlc : IsLocallyConstant f) (hcs : HasCompactSupport f) :
  MemLp f 2 (Measure.pi (fun p : S => @dx_p p ⟨hS p p.2⟩)) := by
    -- If $f$ is continuous and compactly supported, then it's in $L^2$.
    have h_cont_compact : Continuous f ∧ HasCompactSupport f := by
      exact ⟨ hlc.continuous, hcs ⟩;
    have h_mem_L2 : MeasureTheory.Integrable (fun x => ‖f x‖ ^ 2) (Measure.pi (fun p : S => @dx_p p ⟨hS p p.2⟩)) := by
      have h_mem_L2 : ContinuousOn (fun x => ‖f x‖ ^ 2) (closure (Function.support f)) ∧ IsCompact (closure (Function.support f)) := by
        exact ⟨ h_cont_compact.1.norm.continuousOn.pow 2, h_cont_compact.2.isCompact ⟩;
      have h_mem_L2 : MeasureTheory.IntegrableOn (fun x => ‖f x‖ ^ 2) (closure (Function.support f)) (Measure.pi (fun p : S => @dx_p p ⟨hS p p.2⟩)) := by
        apply_rules [ ContinuousOn.integrableOn_compact, h_mem_L2.1 ];
        have h_finite_measure : ∀ p : S, IsFiniteMeasureOnCompacts (@dx_p p ⟨hS p p.2⟩) := by
          unfold dx_p;
          intro p; exact inferInstance;
        infer_instance;
      rw [ MeasureTheory.integrableOn_iff_integrable_of_support_subset ] at h_mem_L2;
      · exact h_mem_L2;
      · exact fun x hx => subset_closure <| by simpa using hx;
    rw [ memLp_two_iff_integrable_sq_norm ] ; aesop;
    exact h_cont_compact.1.aestronglyMeasurable

/-
Proving that the tensor product of two L2 functions is in L2 of the product measure.
-/
open MeasureTheory Complex ENNReal ContinuousLinearMap
...
theorem MemLp_prod_tensor {f : α → ℂ} {g : β → ℂ}
  (hf : MemLp f 2 μ) (hg : MemLp g 2 ν) :
  MemLp (fun p : α × β => f p.1 * g p.2) 2 (Measure.prod μ ν) := by
    -- The product of two L2 functions is again L2.
    have h_prod_L2 : ∀ (f : α → ℂ) (g : β → ℂ), MemLp f 2 μ → MemLp g 2 ν → MemLp (fun p : α × β => f p.1 * g p.2) 2 (μ.prod ν) := by
      intros f g hf hg
      have h_prod : MemLp (fun p : α × β => f p.1 * g p.2) 2 (μ.prod ν) := by
        have h_f : MemLp f 2 μ := hf
        have h_g : MemLp g 2 ν := hg
        rw [ memLp_two_iff_integrable_sq_norm ] at h_f h_g ⊢;
        · simpa [ mul_pow ] using h_f.prod_mul h_g;
        · have h_f : AEStronglyMeasurable f μ := by
            exact hf.1
          have h_g : AEStronglyMeasurable g ν := by
            exact hg.1;
          have h_f : AEStronglyMeasurable (fun p : α × β => f p.1) (μ.prod ν) := by
            exact?
          have h_g : AEStronglyMeasurable (fun p : α × β => g p.2) (μ.prod ν) := by
            exact?;
          exact h_f.mul h_g;
        · exact h_g.1;
        · exact h_f.1
      exact h_prod;
    exact h_prod_L2 f g hf hg

/-
Proving that Schwartz-Bruhat functions are in L2(A_S) using the previous lemmas.
-/
open MeasureTheory Complex ENNReal ContinuousLinearMap

variable (S : Finset ℕ) (hS : ∀ p ∈ S, Nat.Prime p)

theorem SchwartzBruhat_subset_L2 (f : SchwartzBruhatSpace S hS) :
  MemLp f.val 2 (mu_S S hS) := by
    -- By definition of Schwartz-Bruhat functions, each term in the sum is a product of a Schwartz function on ℝ and a locally constant compactly supported function on the finite adeles.
    obtain ⟨n, g, h, hgh, hf⟩ := f.2;
    -- By definition of Schwartz functions, each term in the sum is a product of a Schwartz function on ℝ and a locally constant compactly supported function on the finite adeles. Therefore, each term is in L2(A_S).
    have h_term_mem_L2 : ∀ i : Fin n, MemLp (fun x : A_S S hS => (g i) x.1 * (h i) x.2) 2 (mu_S S hS) := by
      intro i
      have h_term_mem_L2 : MemLp (fun x : ℝ => (g i) x) 2 (volume) ∧ MemLp (fun x : FiniteAdeleS S hS => (h i) x) 2 (Measure.pi (fun p : S => @dx_p p ⟨hS p p.2⟩)) := by
        exact ⟨ Schwartz_mem_L2_real ( g i ), LocallyConstant_CompactSupport_mem_L2_finite_adele S hS ( h i ) ( hgh i |>.1 ) ( hgh i |>.2 ) ⟩;
      convert MemLp_prod_tensor h_term_mem_L2.1 h_term_mem_L2.2;
    rw [ hf ];
    exact?

/-
Defining the Fourier transform on Q_p.
-/
open MeasureTheory Complex ENNReal ContinuousLinearMap

variable (p : ℕ) [Fact p.Prime]

noncomputable def FourierTransform_Qp (f : ℚ_[p] → ℂ) (y : ℚ_[p]) : ℂ :=
  ∫ x, f x * psi_p p (x * y) ∂(dx_p p)
...
noncomputable def FourierTransform_FiniteAdele (f : FiniteAdeleS S hS → ℂ) (y : FiniteAdeleS S hS) : ℂ :=
  ∫ x, f x * (∏ p : S, @psi_p p ⟨hS p p.2⟩ (x p * y p)) ∂(Measure.pi (fun p : S => @dx_p p ⟨hS p p.2⟩))

/-
Checking for convolution and Haar measure.
-/
open MeasureTheory Complex ENNReal ContinuousLinearMap
...
/-
Checking if C_S is a commutative group.
-/
open MeasureTheory Complex ENNReal ContinuousLinearMap
...
#synth CommGroup (C_S S hS)

/-
Stating that the norm on C_S is multiplicative.
-/
open MeasureTheory Complex ENNReal ContinuousLinearMap

variable (S : Finset ℕ) (hS : ∀ p ∈ S, Nat.Prime p)

theorem norm_C_S_mul (x y : C_S S hS) : norm_C_S S hS (x * y) = norm_C_S S hS x * norm_C_S S hS y := by
  -- By definition of $norm_C_S$, we know that $norm_C_S x * norm_C_S y = norm_J_S (x * y)$ for any $x, y \in C_S$.
  have h_norm_mul : ∀ x y : J_S S hS, norm_J_S S hS (x * y) = norm_J_S S hS x * norm_J_S S hS y := by
    simp +decide [ norm_J_S ];
    simp +decide [ norm_A_S ];
    simp +decide [ mul_assoc, mul_comm, mul_left_comm, Finset.prod_mul_distrib ];
  -- By definition of $C_S$, we know that its elements are equivalence classes of ideles under multiplication by $O_S^*$. Therefore, we can lift the elements $x$ and $y$ to $J_S$.
  obtain ⟨x', hx'⟩ : ∃ x' : J_S S hS, x = QuotientGroup.mk x' := by
    induction x using Quotient.inductionOn' ; tauto
  obtain ⟨y', hy'⟩ : ∃ y' : J_S S hS, y = QuotientGroup.mk y' := by
    induction y using Quotient.inductionOn' ; tauto;
  -- Substitute hx' and hy' into the goal.
  rw [hx', hy'];
  exact h_norm_mul x' y' ▸ rfl

/-
Defining the kernel of the operator V_S.
-/
open MeasureTheory Complex ENNReal ContinuousLinearMap

variable (S : Finset ℕ) (hS : ∀ p ∈ S, Nat.Prime p)

noncomputable def V_S_kernel (h : C_S S hS → ℂ) (x y : C_S S hS) : ℂ :=
  h (x * y⁻¹) * (norm_C_S S hS (x * y⁻¹)).sqrt

/-
Defining the application of the operator V_S to a function F.
-/
open MeasureTheory Complex ENNReal ContinuousLinearMap

variable (S : Finset ℕ) (hS : ∀ p ∈ S, Nat.Prime p)

noncomputable def V_S_apply (h : C_S S hS → ℂ) (F : C_S S hS → ℂ) (x : C_S S hS) : ℂ :=
  ∫ y, V_S_kernel S hS h x y * F y ∂(mu_C_S S hS)
